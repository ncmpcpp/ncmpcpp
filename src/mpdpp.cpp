...
bool Connection::AddRandomTag(mpd_tag_type tag, size_t number, const std::string &db_exclude_pattern, std::mt19937 &rng)
{
	...
	auto it = tags.begin();
	boost::regex re(db_exclude_pattern);
	for (size_t i = 0; i < number && it != tags.end(); ++i)
	{
		...
		for (const auto &path : paths)
			if (db_exclude_pattern.empty() || !boost::regex_match(path, re))
				AddSong(path);
		CommitCommandsList();
	}
	return true;
...
bool Connection::AddRandomSongs(size_t number, const std::string &random_exclude_pattern, const std::string &db_exclude_pattern, std::mt19937 &rng)
{
	prechecksNoCommandsList();
	...
	{
		if (db_exclude_pattern.empty() || !boost::regex_match(item->value, re))
