...
//
// toColor: a scaling function for coloring. For numbers 0 to max this function returns a coloring from
// the lowest color to the highest, and colors will not loop from 0 to max.
//
...
//
// DrawSoundEllipseStereo: This visualizer only works in stereo. The colors form concentric rings originating from the center (width/2, height/2).
// For any given point, the width is scaled with the left channel and height is scaled with the right channel. For example,
// if a song is entirely in the right channel, then it would just be a vertical line.
//
// Since every font/terminal is different, the visualizer is never a perfect circle. This visualizer assume the font height
// is twice the length of the font's width. If the font is skinner or wider than this, instead of a circle it will be an ellipse.
//
void Visualizer::DrawSoundEllipseStereo(int16_t *buf_left, int16_t *buf_right, ssize_t samples, size_t height)
{
	...
	//Makes the radius of the color circle proportional to max of height or width. Divide by colors size so that there are
	//multiple color rings instead of just a few.
	...
	for (size_t i = 0; i < samples; ++i)
	{
		...
		//The arguments to the toColor function roughly follow a circle equation where the center is not centered around (0,0)
		//For example (x - w)^2 + (y-h)+2 = r^2 centers the circle around the point (w,h)
		//Because fonts are not all the same size, this will not always generate a perfect circle.
...
//
// DrawSoundWaveFill: This visualizer is very similar to DrawSoundWave, but instead of a single line the entire height is filled. In stereo mode,
// the top half of the screen is dedicated to the right channel, the bottom the left channel.
//
