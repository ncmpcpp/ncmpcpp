...
void AddRandomItems::run()
{
	...
	if (number > 0)
	{
		bool success;
		if (rnd_type == 's')
			success = Mpd.AddRandomSongs(number, Config.random_exclude_pattern, Config.db_exclude_pattern, Global::RNG);
		else
			success = Mpd.AddRandomTag(tag_type, number, Config.db_exclude_pattern, Global::RNG);
		if (success)
